/*
  rules.js — THE RULES ENGINE  🧬
  --------------------------------------------------------------------
  This is the scientific heart of the project: it reasons from the
  PHYSICS to the BIOLOGY. Same pattern as planet.js, but instead of
  deciding colours it decides what life COULD arise — and always shows
  its reasoning ("because ...").

  HONESTY: every conclusion is a POSSIBILITY ("could", "would likely"),
  never a claim that life exists. It returns plain data; main.js draws it.

  Input `c`: { Tsurf, water, atmosphere, retention, star, rotation,
               gravity, radiation, score }
  Output: { level, summary, findings: [ {cat, conclusion, reason} ] }
*/

function generateBiosphere(c) {
  const a = c.atmosphere;

  // ---- Read the physics into simple true/false facts ----
  const hasAtm     = c.retention !== "stripped";
  const liquid     = hasAtm && c.Tsurf >= 273 && c.Tsurf <= 373 && c.water > 0;
  const oxygenRich = a.O2 >= 10;
  const reducing   = (a.CH4 + a.H2) >= 20 && a.O2 < 5;   // oxygen-poor, hydrogen/methane-rich
  const tooHot     = c.Tsurf > 373;
  const tooCold    = c.Tsurf < 245;
  const temperate  = c.Tsurf >= 255 && c.Tsurf <= 330;
  const highRad    = c.radiation >= 6;                   // "high" or "extreme"
  const highG      = c.gravity > 1.5;
  const lowG       = c.gravity < 0.5;
  const tidalLock  = c.star === "M" && c.rotation >= 500; // crude proxy for tidal locking

  const findings = [];

  // ---- 1) ENERGY SOURCE: how would life get its energy? ----
  if (!hasAtm || tooHot) {
    findings.push({
      cat: "Energy source",
      conclusion: "Chemosynthesis, not photosynthesis",
    ...
    findings.push({
      cat: "Energy source",
      conclusion: "Oxygen-based photosynthesis is plausible",
    ...
    findings.push({
      cat: "Energy source",
      conclusion: "Anoxygenic or chemosynthetic life is more likely",
    ...
    findings.push({
      cat: "Energy source",
      conclusion: "Limited, simple light-harvesting life may be possible",
  ...
  // ---- 2) LIGHT & PIGMENTS: shaped by the star (only if surface life is viable) ----
  if (hasAtm && !tooHot) {
    if (c.star === "M") {
      findings.push({
        cat: "Light & pigments",
        conclusion: "Dark red or black photosynthetic pigments",
      ...
      findings.push({
        cat: "Light & radiation",
        conclusion: "Strong UV defences or shelter would be needed",
      ...
      findings.push({
        cat: "Light & pigments",
        conclusion: "Green-type photosynthesis is feasible",
        reason: "This star's light is similar to our Sun's, so Earth-like green pigments could capture it efficiently.",
      });
    }
  }

  // ---- 3) HABITAT: where would life most likely cluster? ----
  if (!hasAtm || highRad) {
    findings.push({
      cat: "Likely habitat",
      conclusion: "Underground or sheltered refuges",
    ...
    findings.push({
      cat: "Likely habitat",
      conclusion: "The twilight 'terminator' ring",
  ...
  // ---- Build an inferred FOOD CHAIN (generic roles now; the AI names them later) ----
  const aspect = highG ? "squat" : lowG ? "upright" : "balanced";

  // Producers: chemosynthetic microbes, or photosynthesisers coloured by the star
  let producer;
  ...
  if (chemoBase) {
    ...
    let pcol = "#3fae6b";                                  // green (Sun-like)
    if (c.star === "M") pcol = "#b03a3a";                  // dark red under a red dwarf
    else if (c.star === "A" || c.star === "F") pcol = "#9b6dd0"; // UV-screened pigments
  ...
  else                          chain = [producer]; // hostile: little more than producers
